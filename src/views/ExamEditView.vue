<template>
  ...
  try {
    ...
    const response = await axios.delete('/exam/deleteQuestion', {
      data: requestBody
    })
    if (response.data.code === 1) {
      emit('deleteQuestion', props.qid)
    } else {
      throw new Error(response.data.msg || '删除题目失败')
    }
  } catch (err) {
    error.value = err.response?.data?.msg || err.message || '删除题目失败'
  } finally {
    loading.value = false
  }
}
